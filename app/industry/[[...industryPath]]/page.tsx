...
export async function generateMetadata({
  ...
  const uid = params.industryPath[params.industryPath.length - 1]

  try {
    const page = await client.getByUID('industry', uid)
...
export async function generateStaticParams() {
  ...
  const pages = await client.getAllByType('industry')

  return pages.map((page) => ({
    industryPath: page.url?.split('/').filter(Boolean) || []
  }))
}
