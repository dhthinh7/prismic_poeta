...
export async function generateMetadata({
  ...
  const uid = params.partnershipsPath[params.partnershipsPath.length - 1]

  try {
    const page = await client.getByUID('partnerships', uid)
...
export async function generateStaticParams() {
  ...
  const pages = await client.getAllByType('partnerships')

  return pages.map((page) => ({
    partnershipsPath: page.url?.split('/').filter(Boolean) || []
  }))
}
